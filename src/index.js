...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  return response.status(403).json({error: "User has 10 todos and is not Pro!"})
}
...
function checksTodoExists(request, response, next) {
  ...
  const userHasThisTodo = user.todos.find(todo => todo.id === id);
